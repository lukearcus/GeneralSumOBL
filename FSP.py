...
def run(opts):
    ...
    worker = FSP_algo.FSP(game_obj, agents, max_iters=iters, max_time=time, m=num_BR, n=num_mixed, exploit_freq=check_freq)
    pi, exploitability, data = worker.run_algo()
    
    #FSP_plots(exploitability, worker.est_exploit_freq, [pi], 'kuhn') 
    res.append({"exploit":exploitability, "extra_data":data})
    return res
